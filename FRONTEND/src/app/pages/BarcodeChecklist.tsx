...
import { barcodeChecklistAPI } from "../../api";

interface BarcodeChecklistProps {
  onLogout: () => void;
  onBack: () => void;
  onNext?: () => void;
...
export function BarcodeChecklist({
  onLogout,
  onBack,
  onNext,
  initialHanger,
}: BarcodeChecklistProps) {
  ...
  const handleHangerSelect = async (hangerNo: number) => {
    ...
    try {
      const response = await barcodeChecklistAPI.getForHanger(hangerNo);
      if (response.success) {
        setChecklistData(
      ...
      console.error("Failed to fetch barcode checklist:", err);
      // Fallback to default checklist for Barcode
      setChecklistData([
        { sr: 1, activity: "Barcode Readable", status: "pending", remarks: "", image: "", standard_value: "" },
        { sr: 2, activity: "Barcode Visible", status: "pending", remarks: "", image: "", standard_value: "" },
        {
          sr: 3,
          activity: "Barcode Not Damaged",
          status: "pending",
          remarks: "",
          image: "",
          standard_value: "",
        ...
        {
          sr: 4,
          activity: "Barcode Position Correct",
          status: "pending",
          remarks: "",
          image: "",
          standard_value: "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await barcodeChecklistAPI.save(
        selectedHanger!,
        checklistPayload,
        doneBy,
        doneOn
      ...
      if (response.success) {
        alert(
          `Barcode Checklist submitted successfully for Hanger ${selectedHanger}`
        );
      ...
      alert(err.message || "Failed to submit barcode checklist");
    } finally {
          ...
          >
            <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
              <h2 className="text-2xl font-bold text-gray-800 mb-4">
                Hanger {selectedHanger} - Barcode Checklist
              </h2>
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {checklistData.map((item, index) => (
                      <tr key={`checklist-${item.sr}-${index}`} className="hover:bg-gray-50">
              ...
              <div className="mt-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                ...
                <button
                  ...
                  className="px-8 py-3 bg-gray-700 text-white text-lg font-semibold rounded-xl hover:bg-gray-800 transition-all shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                >
